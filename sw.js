// slices.js - ПОЛНАЯ ИСПРАВЛЕННАЯ ВЕРСИЯ (без дублирования постов)
...
var loadedSliceIds = new Set(); // ДЛЯ ОТСЛЕЖИВАНИЯ ЗАГРУЖЕННЫХ ПОСТОВ
...
function loadSlices() {
    ...
    // Сначала получаем все существующие посты через once()
    ...
    // Теперь слушаем ТОЛЬКО новые посты
    ...
    slicesListener.on('child_added', function(snapshot) {
        ...
        // ПРОВЕРЯЕМ: если пост уже загружен - пропускаем (избегаем дублей)
        if (loadedSliceIds.has(sliceId)) {
...
async function publishSlice() {
    ...
    try {
        ...
        // Очищаем Set и перезагружаем ленту, чтобы не было дублей
        loadedSliceIds.clear();
        loadSlices();
...
window.loadSlices = loadSlices;

console.log('✅ slices.js полностью загружен (исправлена версия без дублирования)');
