...
function loadReels() {
    ...
    query.once('value').then(function(snapshot) {
        ...
        var arr = Object.keys(reels).map(function(id) { return { id: id, data: reels[id] }; });
        arr.sort(function(a, b) { return (b.data.createdAt || 0) - (a.data.createdAt || 0); });
        arr.forEach(function(reel) { feed.appendChild(createReelCard(reel.id, reel.data)); });
    }).catch(function(err) { feed.innerHTML = '<div class="empty-reels"><span>❌</span><p>Ошибка</p></div>'; });
}
...
function createReelCard(reelId, reelData) {
    ...
    div.innerHTML = '<img src="' + reelData.mediaUrl + '">' +
        '<div class="reel-card-overlay"><div class="reel-card-stats"><span>❤️ ' + likes + '</span><span>💬 ' + comments + '</span></div></div>';
    return div;
}
...
function publishReel() {
    ...
    uploadImageToImgBB(currentReelFile).then(function(data) {
    ...
    }).then(function() { closeCreateReelModal(); showNotification('Реелс опубликован!', 'success'); loadReels(); }).catch(function(err) { showNotification('Ошибка', 'error'); });
}
function viewReel(reelId, reelData) {
    ...
    database.ref('reels/' + reelId + '/viewsCount').transaction(function(v) { return (v || 0) + 1; });
    checkIfLiked(reelId);
}
...
function checkIfLiked(reelId) {
    database.ref('reelLikes/' + reelId + '/' + currentUser.uid).once('value').then(function(snap) {
...
function likeReel() {
    ...
    likeRef.once('value').then(function(snap) {
        if (snap.exists()) {
            likeRef.remove();
            countRef.transaction(function(c) { return Math.max((c || 1) - 1, 0); });
            document.getElementById('like-reel-btn').classList.remove('liked');
        } else {
            likeRef.set(true);
            countRef.transaction(function(c) { return (c || 0) + 1; });
            document.getElementById('like-reel-btn').classList.add('liked');
        }
        countRef.once('value').then(function(s) { document.getElementById('reel-likes-count').textContent = s.val() || 0; });
    });
}
...
function deleteCurrentReel() {
    if (!viewingReelId) return;
    database.ref('reels/' + viewingReelId).once('value').then(function(snap) {
        var reel = snap.val();
        if (reel.authorId === currentUser.uid || isSuperAdmin) {
            if (confirm('Удалить реелс?')) { database.ref('reels/' + viewingReelId).remove().then(function() { showNotification('Удалён', 'success'); closeViewReelModal(); loadReels(); }); }
