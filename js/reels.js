// KUKUMBER MESSENGER - REELS (исправленный)
...
function loadReels() {
    ...
    query.once('value').then(function(snapshot) {
        ...
        feed.innerHTML = '';
        if (!reels) {
            feed.innerHTML = '<div class="empty-reels"><span>🎬</span><p>Пока нет реелсов</p></div>';
            return;
        }
        var arr = [];
        for (var id in reels) {
            arr.push({ id: id, data: reels[id] });
        }
        arr.sort(function(a, b) {
            return (b.data.createdAt || 0) - (a.data.createdAt || 0);
        });
        for (var i = 0; i < arr.length; i++) {
            feed.appendChild(createReelCard(arr[i].id, arr[i].data));
        }
    }).catch(function(err) {
        console.error(err);
...
function loadLikedReels() {
    var feed = document.getElementById('reels-feed');
    if (feed) feed.innerHTML = '<div class="empty-reels"><span>❤️</span><p>В разработке</p></div>';
}
...
function previewReelMedia(event) {
    ...
    if (!file.type.startsWith('image/')) {
        showNotification('Только фото', 'error');
        return;
    }
...
function publishReel() {
    if (!currentReelFile) {
        showNotification('Выберите фото', 'error');
        return;
    }
...
function viewReel(reelId, reelData) {
    ...
    if (reelData.authorAvatar) {
        avatarDiv.style.backgroundImage = 'url(' + reelData.authorAvatar + ')';
...
function likeReel() {
    ...
    likeRef.once('value').then(function(snap) {
        ...
        countRef.once('value').then(function(s) {
            document.getElementById('reel-likes-count').textContent = s.val() || 0;
        });
    });
}
...
function shareReel() {
    if (navigator.share) {
        navigator.share({ title: 'Реелс Kukumber', text: 'Посмотри!', url: window.location.href });
    } else {
        showNotification('Скопируйте ссылку', 'info');
    }
}
    ...
    database.ref('reels/' + viewingReelId).once('value').then(function(snap) {
        ...
        if (reel.authorId === currentUser.uid || isSuperAdmin) {
            if (confirm('Удалить реелс?')) {
                database.ref('reels/' + viewingReelId).remove().then(function() {
