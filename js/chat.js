...
var unreadMessages = {}; // { chatId: количество }
var lastReadTimes = {}; // { chatId: timestamp }

// Загрузка непрочитанных сообщений
async function loadUnreadCounts() {
    if (!currentUser || !currentUser.uid) return;
    
    const unreadSnap = await database.ref('unread/' + currentUser.uid).once('value');
    unreadMessages = unreadSnap.val() || {};
    
    // Обновляем отображение в списке чатов
    updateAllUnreadBadges();
}

// Обновление бейджа непрочитанных у чата
function updateUnreadBadge(chatId) {
    ...
    if (chatItem) {
        ...
        if (count > 0) {
            const previewDiv = chatItem.querySelector('.chat-item-preview');
            if (previewDiv) {
                const badge = document.createElement('span');
                badge.className = 'unread-badge';
                badge.textContent = count > 99 ? '99+' : count;
                previewDiv.parentNode.insertBefore(badge, previewDiv.nextSibling);
            }
        }
        
        // Добавляем активный класс для выделения
        if (count > 0) {
...
async function markMessageAsRead(chatId, messageId) {
    ...
    // Обновляем последнее прочитанное время
    await database.ref('userRead/' + currentUser.uid + '/' + chatId).set(messageTime);
    
    // Очищаем непрочитанные для этого чата
    if (unreadMessages[chatId]) {
        delete unreadMessages[chatId];
        await database.ref('unread/' + currentUser.uid + '/' + chatId).remove();
        updateUnreadBadge(chatId);
    }
}

// Отметить все сообщения в чате как прочитанные при открытии
async function markChatAsRead(chatId) {
    if (!currentUser || !currentUser.uid) return;
    
    await database.ref('userRead/' + currentUser.uid + '/' + chatId).set(firebase.database.ServerValue.TIMESTAMP);
    ...
    if (unreadMessages[chatId]) {
        ...
        // Обновляем позицию чата в списке (поднимаем вверх)
        moveChatToTop(chatId);
    }
}

// Поднять чат вверх в списке
function moveChatToTop(chatId) {
    const chatItem = document.querySelector(`.chat-item[data-chat-id="${chatId}"]`);
    const chatsList = document.getElementById('chats-list');
    
    if (chatItem && chatsList && chatsList.firstChild !== chatItem) {
        chatsList.insertBefore(chatItem, chatsList.firstChild);
    }
}

// ========== РАЗДЕЛИТЕЛЬ СТАРЫХ/НОВЫХ СООБЩЕНИЙ ==========
var lastReadTimestamp = null;
var newMessagesCount = 0;
var hasScrolledToNew = false;

// Загрузка последнего прочитанного времени
async function loadLastReadTime(chatId) {
    if (!currentUser || !currentUser.uid) return null;
    
    const readSnap = await database.ref('userRead/' + currentUser.uid + '/' + chatId).once('value');
    lastReadTimestamp = readSnap.val() || null;
    return lastReadTimestamp;
}

// Добавление разделителя в сообщения
function addMessagesDivider() {
    const container = document.getElementById('messages-container');
    if (!container) return;
    
    // Удаляем старый разделитель
    const oldDivider = container.querySelector('.messages-divider');
    if (oldDivider) oldDivider.remove();
    
    if (!lastReadTimestamp) return;
    
    // Находим первое новое сообщение
    const messages = container.querySelectorAll('.message');
    let firstNewMessage = null;
    
    for (const msg of messages) {
        const msgTime = parseInt(msg.getAttribute('data-timestamp'));
    ...
    if (firstNewMessage) {
        ...
        divider.innerHTML = `
            <div class="messages-divider-line"></div>
            <div class="messages-divider-text">📌 Новые сообщения</div>
            <div class="messages-divider-line"></div>
        `;
        firstNewMessage.parentNode.insertBefore(divider, firstNewMessage);
    }
}
...
function countNewMessages() {
    const container = document.getElementById('messages-container');
    if (!container || !lastReadTimestamp) return 0;
...
function updateScrollButton() {
    ...
    if (!isNearBottom && newMessagesCount > 0) {
        if (!btn) {
            ...
            btn.innerHTML = `↓<span class="unread-count">${newMessagesCount}</span>`;
            btn.onclick = () => {
            ...
            const countSpan = btn.querySelector('.unread-count');
            if (countSpan) countSpan.textContent = newMessagesCount;
        }
    } else if (btn) {
...
// Прокрутка к разделителю (границе новых сообщений)
function scrollToNewMessages() {
    const container = document.getElementById('messages-container');
    const divider = container?.querySelector('.messages-divider');
    
    if (divider) {
        divider.scrollIntoView({ behavior: 'smooth', block: 'center' });
    } else if (container && newMessagesCount > 0) {
        container.scrollTop = container.scrollHeight;
    }
}

// Обновлённая функция добавления сообщения с timestamp
function appendMessage(message) {
    var container = document.getElementById('messages-container');
    if (!container) return;
    ...
    // ... остальной код appendMessage (как был) ...
    
    container.appendChild(messageDiv);
    
    // Проверяем нужно ли прокрутить
    const isNearBottom = container.scrollHeight - container.scrollTop - container.clientHeight < 100;
    if (isNearBottom || isSent) {
        container.scrollTop = container.scrollHeight;
    } else if (!hasScrolledToNew && !isSent) {
        // Если есть новые сообщения и мы не прокручивались - показываем кнопку
        countNewMessages();
    }
...
// Обновлённая функция открытия чата
async function openChatWithData(chatId, chatData) {
    console.log('openChatWithData:', chatId, chatData.type);
    ...
    // Загружаем последнее прочитанное время
    await loadLastReadTime(chatId);
    ...
    // Через небольшую задержку прокручиваем к новым сообщениям
    setTimeout(() => {
        scrollToNewMessages();
